...
test('both general and contest official evidence remain available offline',async({page,context})=>{
  ...
  const cached=await page.evaluate(async()=>{
    const a=await fetch('./data/official-evidence.json').then(r=>r.json());
    const b=await fetch('./data/contest-official-evidence.json').then(r=>r.json());
    return{base:a.rules?.length||0,contest:b.rules?.length||0};
  });
